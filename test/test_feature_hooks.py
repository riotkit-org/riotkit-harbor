...
SIMPLE_ENV_HOOKS_DIR = os.path.dirname(os.path.realpath(__file__)) + '/testdata/env_simple/hooks.d'


class TestHooksFeature(BaseHarborTestClass):
    def _prepare_test_data(self):
        with open(SIMPLE_ENV_HOOKS_DIR + '/pre-upgrade/whoami.sh', 'w') as fw:
            fw.write('''
                #!/bin/bash
                echo " >> This is a whoami.sh hook, test: $(whoami)"
            ''')

        with open(SIMPLE_ENV_HOOKS_DIR + '/post-upgrade/history.sh', 'w') as fw:
            fw.write('''
                #!/bin/bash
        ...
        subprocess.check_call(['chmod', '+x', SIMPLE_ENV_HOOKS_DIR + '/pre-upgrade/whoami.sh'])
        subprocess.check_call(['chmod', '+x', SIMPLE_ENV_HOOKS_DIR + '/post-upgrade/history.sh'])
